...
class CRUDEmpresaMembro:
    ...
    def get(self, db: Session, membro_id: int):
        membro = db.query(EmpresaMembros).filter(EmpresaMembros.membro_id == membro_id).first()
        if not membro:
            raise HTTPException(404, "Membro não encontrado")
        return membro

    def get_by_empresa(self, db: Session, empresa_id: int):
        return db.query(EmpresaMembros).filter(EmpresaMembros.empresa_id == empresa_id).all()

    def create(self, db: Session, data: EmpresaMembroCreate):
        # Respeita UNIQUE (empresa_id, usuario_id)
        existente = db.query(EmpresaMembros).filter(
            EmpresaMembros.empresa_id == data.empresa_id,
            EmpresaMembros.usuario_id == data.usuario_id,
        ).first()
