...
def listar_feriados(db: Session = Depends(get_db), current_user=Depends(check_auth)):
    return crud_feriados.listar(db)


@router.get("/{feriado_id}", response_model=FeriadoOut)
def obter_feriado(feriado_id: str, db: Session = Depends(get_db), current_user=Depends(check_auth)):
    return crud_feriados.get(db, feriado_id)


@router.post("/", response_model=FeriadoOut, status_code=201)
def criar_feriado(data: FeriadoCreate, db: Session = Depends(get_db), current_user=Depends(check_auth)):
    return crud_feriados.criar(db, data)


@router.put("/{feriado_id}", response_model=FeriadoOut)
def atualizar_feriado(feriado_id: str, data: FeriadoUpdate, db: Session = Depends(get_db), current_user=Depends(check_auth)):
    return crud_feriados.atualizar(db, feriado_id, data)


@router.delete("/{feriado_id}")
def deletar_feriado(feriado_id: str, db: Session = Depends(get_db), current_user=Depends(check_auth)):
