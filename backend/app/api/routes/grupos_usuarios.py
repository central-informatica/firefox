...
def obter(grupo_usuario_id: str, db: Session = Depends(get_db), current_user=Depends(check_auth)):
    return crud_grupos_usuarios.get(db, grupo_usuario_id)


@router.post("/", response_model=GrupoUsuarioOut, status_code=201)
def criar(data: GrupoUsuarioCreate, db: Session = Depends(get_db), current_user=Depends(check_auth)):
    return crud_grupos_usuarios.criar(db, data)


@router.post("/bulk", response_model=GrupoUsuarioBulkResult)
def criar_bulk(payload: GrupoUsuarioBulkCreate, db: Session = Depends(get_db), current_user=Depends(check_auth)):
...
def atualizar(grupo_usuario_id: str, data: GrupoUsuarioUpdate, db: Session = Depends(get_db), current_user=Depends(check_auth)):
    return crud_grupos_usuarios.atualizar(db, grupo_usuario_id, data)


@router.delete("/{grupo_usuario_id}")
def deletar(grupo_usuario_id: str, db: Session = Depends(get_db), current_user=Depends(check_auth)):
