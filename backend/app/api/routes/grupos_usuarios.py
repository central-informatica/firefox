...
def obter(grupo_usuario_id: str, db: Session = Depends(get_db), current_user=Depends(check_auth_with_ip)):
    if not current_user.get("is_admin"):
        raise HTTPException(status_code=403, detail="Apenas administradores podem visualizar grupos-usuarios")
    return crud_grupos_usuarios.get(db, grupo_usuario_id)


@router.post("/", response_model=GrupoUsuarioOut, status_code=201)
def criar(data: GrupoUsuarioCreate, db: Session = Depends(get_db), current_user=Depends(check_auth_with_ip)):
    if not current_user.get("is_admin"):
        raise HTTPException(status_code=403, detail="Apenas administradores podem criar grupos-usuarios")
    return crud_grupos_usuarios.criar(db, data)


@router.post("/bulk", response_model=GrupoUsuarioBulkResult)
def criar_bulk(payload: GrupoUsuarioBulkCreate, db: Session = Depends(get_db), current_user=Depends(check_auth_with_ip)):
    """Cria vínculos em lote entre um grupo e múltiplos usuários. Retorna os ids criados e os pulados com razão."""
    if not current_user.get("is_admin"):
        raise HTTPException(status_code=403, detail="Apenas administradores podem criar grupos-usuarios em lote")
...
def atualizar(grupo_usuario_id: str, data: GrupoUsuarioUpdate, db: Session = Depends(get_db), current_user=Depends(check_auth_with_ip)):
    if not current_user.get("is_admin"):
        raise HTTPException(status_code=403, detail="Apenas administradores podem atualizar grupos-usuarios")
    return crud_grupos_usuarios.atualizar(db, grupo_usuario_id, data)


@router.delete("/{grupo_usuario_id}")
def deletar(grupo_usuario_id: str, db: Session = Depends(get_db), current_user=Depends(check_auth_with_ip)):
    if not current_user.get("is_admin"):
        raise HTTPException(status_code=403, detail="Apenas administradores podem deletar grupos-usuarios")
