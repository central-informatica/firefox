...
def criar(data: RegraAcessoHostCreate, db: Session = Depends(get_db), current_user=Depends(check_auth)):
    return crud_regras_acesso_hosts.criar(db, data)


@router.put("/{regra_id}", response_model=RegraAcessoHostOut)
def atualizar(regra_id: str, data: RegraAcessoHostUpdate, db: Session = Depends(get_db), current_user=Depends(check_auth)):
