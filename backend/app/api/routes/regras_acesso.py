...
@router.get("/empresa/{empresa_id}", response_model=list[RegraAcessoOut])
def listar_empresa(empresa_id: int, db: Session = Depends(get_db)):
    return crud_regras_acesso.listar_por_empresa(db, empresa_id)
...
def criar(data: RegraAcessoCreate, db: Session = Depends(get_db)):
    return crud_regras_acesso.criar(db, data)


@router.put("/{regra_id}", response_model=RegraAcessoOut)
def atualizar(regra_id: int, data: RegraAcessoUpdate, db: Session = Depends(get_db)):
