from fastapi import APIRouter, Depends, HTTPException, Query
from sqlalchemy.orm import Session

from backend.app.api.deps import check_auth_with_ip
from backend.app.db.session import get_db
from backend.app.db.models import RegrasAcessoIps, Grupos
from backend.app.schemas.regras_acesso_ips import (
    RegraAcessoIpsCreate,
    RegraAcessoIpsCreateBulk,
    RegraAcessoIpsUpdate,
    RegraAcessoIpsOut
)
from backend.app.crud.regras_acesso_ips import crud_regras_acesso_ips


router = APIRouter(prefix="/regras-acesso-ips", tags=["Regras de Acesso IPs"])


@router.get("/")
def listar_regras(db: Session = Depends(get_db), current_user=Depends(check_auth_with_ip)):
    """Lista todas as regras de acesso IPs."""
    return crud_regras_acesso_ips.listar(db)


@router.get("/empresa/{empresa_id}")
def listar_regras_por_empresa(
    empresa_id: str,
    db: Session = Depends(get_db),
    current_user=Depends(check_auth_with_ip),
    page: int = Query(1, ge=1),
    limit: int = Query(10, ge=1, le=100),
    search: str = "",
):
    """Lista regras de acesso IPs de uma empresa com paginação."""
    query = db.query(RegrasAcessoIps).filter(RegrasAcessoIps.empresa_id == empresa_id)

    # Aplicar filtro de busca pelo IP (cast JSONB to text para busca ILIKE)
    if search:
        from sqlalchemy import cast, Text
        search_term = f"%{search}%"
        query = query.filter(cast(RegrasAcessoIps.ip_addresses, Text).ilike(search_term))

    # Ordenar por data de criação
    query = query.order_by(RegrasAcessoIps.criado_em.desc())

    # Total antes da paginação
    total = query.count()

    # Aplicar paginação
    offset = (page - 1) * limit
    items = query.offset(offset).limit(limit).all()

    # Buscar nomes dos grupos para enriquecer a resposta
    result_data = []
    for r in items:
        grupo = db.query(Grupos).filter(Grupos.grupo_id == r.grupo_id).first()

        result_data.append({
            "regra_id": str(r.regra_id),
            "empresa_id": str(r.empresa_id),
            "grupo_id": str(r.grupo_id),
            "ip_addresses": r.ip_addresses,  # Lista de IPs/CIDRs
            "tipo_dia": r.tipo_dia,
            "dias_especificos": r.dias_especificos,
            "horarios": r.horarios,
            "bloquear_em_feriado": r.bloquear_em_feriado,
            "ativo": r.ativo,
            "criado_em": str(r.criado_em) if r.criado_em else None,
            "grupo_nome": grupo.nome if grupo else None,
        })

    return {
        "data": result_data,
        "total": total,
        "page": page,
        "limit": limit,
    }


@router.get("/grupo/{grupo_id}")
def listar_regras_por_grupo(
    grupo_id: str,
    db: Session = Depends(get_db),
    current_user=Depends(check_auth_with_ip)
):
    """Lista regras de acesso IPs de um grupo específico."""
    items = crud_regras_acesso_ips.listar_por_grupo(db, grupo_id)

    result_data = []
    for r in items:
        result_data.append({
            "regra_id": str(r.regra_id),
            "empresa_id": str(r.empresa_id),
            "grupo_id": str(r.grupo_id),
            "ip_addresses": r.ip_addresses,
            "tipo_dia": r.tipo_dia,
            "dias_especificos": r.dias_especificos,
            "horarios": r.horarios,
            "bloquear_em_feriado": r.bloquear_em_feriado,
            "ativo": r.ativo,
            "criado_em": str(r.criado_em) if r.criado_em else None,
        })

    return result_data


@router.get("/id/{regra_id}")
def obter_regra(regra_id: str, db: Session = Depends(get_db), current_user=Depends(check_auth_with_ip)):
    """Busca uma regra de acesso IP pelo ID."""
    r = crud_regras_acesso_ips.get(db, regra_id)

    grupo = db.query(Grupos).filter(Grupos.grupo_id == r.grupo_id).first()

    return {
        "regra_id": str(r.regra_id),
        "empresa_id": str(r.empresa_id),
        "grupo_id": str(r.grupo_id),
        "ip_addresses": r.ip_addresses,
        "tipo_dia": r.tipo_dia,
        "dias_especificos": r.dias_especificos,
        "horarios": r.horarios,
        "bloquear_em_feriado": r.bloquear_em_feriado,
        "ativo": r.ativo,
        "criado_em": str(r.criado_em) if r.criado_em else None,
        "grupo_nome": grupo.nome if grupo else None,
    }


@router.post("/", status_code=201)
def criar_regra(data: RegraAcessoIpsCreate, db: Session = Depends(get_db), current_user=Depends(check_auth_with_ip)):
    """Cria uma nova regra de acesso IP."""
    if not current_user.get("is_admin"):
        raise HTTPException(status_code=403, detail="Apenas administradores podem criar regras de acesso")

    r = crud_regras_acesso_ips.criar(db, data)

    grupo = db.query(Grupos).filter(Grupos.grupo_id == r.grupo_id).first()

    return {
        "regra_id": str(r.regra_id),
        "empresa_id": str(r.empresa_id),
        "grupo_id": str(r.grupo_id),
        "ip_addresses": r.ip_addresses,
        "tipo_dia": r.tipo_dia,
        "dias_especificos": r.dias_especificos,
        "horarios": r.horarios,
        "bloquear_em_feriado": r.bloquear_em_feriado,
        "ativo": r.ativo,
        "criado_em": str(r.criado_em) if r.criado_em else None,
        "grupo_nome": grupo.nome if grupo else None,
    }


@router.post("/bulk", status_code=201)
def criar_regras_bulk(data: RegraAcessoIpsCreateBulk, db: Session = Depends(get_db), current_user=Depends(check_auth_with_ip)):
    """Cria regras de acesso IP para múltiplos grupos."""
    if not current_user.get("is_admin"):
        raise HTTPException(status_code=403, detail="Apenas administradores podem criar regras de acesso")

    result = crud_regras_acesso_ips.criar_bulk(db, data)

    # Formatar resposta com detalhes
    criadas_formatadas = []
    for r in result["criadas"]:
        grupo = db.query(Grupos).filter(Grupos.grupo_id == r.grupo_id).first()

        criadas_formatadas.append({
            "regra_id": str(r.regra_id),
            "empresa_id": str(r.empresa_id),
            "grupo_id": str(r.grupo_id),
            "ip_addresses": r.ip_addresses,
            "tipo_dia": r.tipo_dia,
            "dias_especificos": r.dias_especificos,
            "horarios": r.horarios,
            "bloquear_em_feriado": r.bloquear_em_feriado,
            "ativo": r.ativo,
            "criado_em": str(r.criado_em) if r.criado_em else None,
            "grupo_nome": grupo.nome if grupo else None,
        })

    return {
        "criadas": criadas_formatadas,
        "total_criadas": result["total_criadas"],
        "erros": result["erros"]
    }


@router.put("/{regra_id}")
def atualizar_regra(regra_id: str, data: RegraAcessoIpsUpdate, db: Session = Depends(get_db), current_user=Depends(check_auth_with_ip)):
    """Atualiza uma regra de acesso IP."""
    if not current_user.get("is_admin"):
        raise HTTPException(status_code=403, detail="Apenas administradores podem atualizar regras de acesso")

    r = crud_regras_acesso_ips.atualizar(db, regra_id, data)

    grupo = db.query(Grupos).filter(Grupos.grupo_id == r.grupo_id).first()

    return {
        "regra_id": str(r.regra_id),
        "empresa_id": str(r.empresa_id),
        "grupo_id": str(r.grupo_id),
        "ip_addresses": r.ip_addresses,
        "tipo_dia": r.tipo_dia,
        "dias_especificos": r.dias_especificos,
        "horarios": r.horarios,
        "bloquear_em_feriado": r.bloquear_em_feriado,
        "ativo": r.ativo,
        "criado_em": str(r.criado_em) if r.criado_em else None,
        "grupo_nome": grupo.nome if grupo else None,
    }


@router.patch("/{regra_id}/toggle")
def toggle_ativo(regra_id: str, db: Session = Depends(get_db), current_user=Depends(check_auth_with_ip)):
    """Alterna o estado ativo/inativo de uma regra de acesso IP."""
    if not current_user.get("is_admin"):
        raise HTTPException(status_code=403, detail="Apenas administradores podem alterar regras de acesso")

    r = crud_regras_acesso_ips.toggle_ativo(db, regra_id)

    grupo = db.query(Grupos).filter(Grupos.grupo_id == r.grupo_id).first()

    return {
        "regra_id": str(r.regra_id),
        "empresa_id": str(r.empresa_id),
        "grupo_id": str(r.grupo_id),
        "ip_addresses": r.ip_addresses,
        "tipo_dia": r.tipo_dia,
        "dias_especificos": r.dias_especificos,
        "horarios": r.horarios,
        "bloquear_em_feriado": r.bloquear_em_feriado,
        "ativo": r.ativo,
        "criado_em": str(r.criado_em) if r.criado_em else None,
        "grupo_nome": grupo.nome if grupo else None,
    }


@router.delete("/{regra_id}")
def deletar_regra(regra_id: str, db: Session = Depends(get_db), current_user=Depends(check_auth_with_ip)):
    """Remove uma regra de acesso IP."""
    if not current_user.get("is_admin"):
        raise HTTPException(status_code=403, detail="Apenas administradores podem deletar regras de acesso")
    return crud_regras_acesso_ips.deletar(db, regra_id)
